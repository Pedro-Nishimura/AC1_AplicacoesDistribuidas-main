...
def evolucao_anterior(nome):
    ...
    if resposta.status_code != 200: raise Exception(f"{resposta.status_code} - {resposta.text}")
    resposta = resposta.json()
    if resposta['evolves_from_species']['name'] != '':
        return resposta['evolves_from_species']['name']
    else:
        return None
...
def nivel_do_pokemon(nome, experiencia):
    ...
    if experiencia resposta2['formula'] < 0:
        raise ValueError
    for x in range(0, len(resposta2['levels']), 1):
        if x['experience'] == experiencia:
            return x['level']
